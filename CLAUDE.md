...
**Data-driven map, not hardcoded scene logic.** `assets/maps/town.json` is GENERATED — edit and rerun `scripts/gen_town_map.py`, never hand-edit the JSON. The generator runs a reachability guard on its own output before it's done (`scripts/check_reachability.py`, importable or standalone): a BFS from `PlayerSpawn` to every `Triggers` rect over the real collision data (tile `collides` properties + `BuildingFootprints` rects), modeling the player's real ~18x14px body — sideways steps only need both tiles open, but crossing a row boundary needs a 2-tile-wide window at both rows, since the body is wider than one 16px tile. It runs twice (any terrain, and paved-network-only excluding grass) and exits non-zero with the unreachable trigger names printed if either fails; `gen_town_map.py` propagates that failure. This exists because a gate can look fine in isolation while opening directly onto a building footprint with no way around it (fixed 2026-07 in the industrial yard — the twin_silo footprint sat right behind its own gate) — don't loosen or skip this check when moving footprints/fences/triggers. It's a Tiled-format map with five layers: `Ground` (autotiled grass/paths/plaza/water — water tiles collide via the `collides` custom property), `Buildings` (set dressing: trees, lamps, benches, fences — most of it collides), and three object layers — `Objects` (spawn point, signpost position), `Triggers` (interaction zones), `BuildingFootprints` (building art anchor + a static collision body per footprint, created in `TownScene` whether or not the art loaded). `TownScene.create()` reads all of this at runtime; there's no per-building code path. To add/move/resize a building or trigger, change the `FOOTPRINTS`/`TRIGGERS` tables in `gen_town_map.py` and regenerate — and keep footprint sizes in lockstep with the sprite sizes in `assets/higgsfield-spec.md`, because `TownScene` renders building PNGs at native size, bottom-center-anchored, depth-sorted against the player (`depth = baseline y`).
...
**Touch support rides the same input paths, never parallel ones.** `TownScene.setupTouchMovement` is an invisible d-pad: the screen splits into 4 triangles along its diagonals, hold-to-walk, first touch wins, and it merges into the same velocity math as WASD (both always active). The floating prompt doubles as the touch action button — tapping it calls the same `triggerInteraction()` as the E key, and `window.open` must stay synchronously inside that pointerup handler or mobile popup blockers eat the redirect. Prompt labels come from `this.promptVerb` ("Press E" / "Tap"), keyed off `game.device.input.touch` once per scene. The canvas is `Scale.FIT` letterboxed (config in `main.js`); `main.js` also sets `body.touch` on touch devices, which index.html CSS uses to grow directory-overlay tap targets to 44px+. Mobile regression: `node scripts/mobile_test.js` (touch-emulated Chrome, both orientations) alongside the desktop `scripts/regression_test.js` — run both after touching input, prompt, or scale code.
